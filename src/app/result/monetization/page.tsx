...
import type { Monetization } from '@/types/validation'
...
function MonetizationContent() {
  ...
  const [section, setSection] = useState<Monetization | null>(null)
  ...
  useEffect(() => {
    ...
    fetch(`/api/result/${resultId}`)
      ...
      .then(data => {
        if (data.success && data.data?.monetization) {
          setSection(data.data.monetization)
        }
      })
      .catch(() => {})
        ...
        <Card className="border-slate-200 bg-white shadow-lg">
          <CardHeader className="pb-4">
            <div className="flex items-start justify-between">
              <div>
                <Badge className="mb-2" variant="outline">Revenue</Badge>
                <CardTitle className="text-2xl">Monetization</CardTitle>
              </div>
          ...
          <CardContent className="space-y-6">
            ...
            <Subsection title="Sales Channels">
              <BulletList items={section.salesChannels} emptyLabel="No sales channels defined." />
            </Subsection>

            <Field label="Projections" value={section.projections} />

            <Subsection title="Key Assumptions">
              <BulletList items={section.keyAssumptions} emptyLabel="No assumptions listed." />
            </Subsection>
          </CardContent>
        </Card>
      </div>
