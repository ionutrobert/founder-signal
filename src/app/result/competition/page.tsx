...
import type { Competition, CompetitorProfile } from '@/types/validation'
...
function CompetitionContent() {
  ...
  const [section, setSection] = useState<Competition | null>(null)
    ...
    fetch(`/api/result/${resultId}`)
      ...
      .then(data => {
        if (data.success && data.data?.competition) {
          setSection(data.data.competition)
        }
      })
      .catch(() => {})
        ...
        <Card className="border-slate-200 bg-white shadow-lg">
          <CardHeader className="pb-4">
            <div className="flex items-start justify-between">
              <div>
                <Badge className="mb-2" variant="outline">Landscape</Badge>
                <CardTitle className="text-2xl">Competition</CardTitle>
              </div>
          ...
          <CardContent className="space-y-6">
            <CompetitorGroup
              ...
              competitors={section.directCompetitors}
              emptyLabel="No direct competitors listed."
            />
            <CompetitorGroup
              title="Indirect Competitors"
              competitors={section.indirectCompetitors}
