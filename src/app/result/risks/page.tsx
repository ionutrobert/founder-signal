...
import type { Risks } from '@/types/validation'
...
function RisksContent() {
  ...
  const [section, setSection] = useState<Risks | null>(null)
  ...
  useEffect(() => {
    ...
    fetch(`/api/result/${resultId}`)
      ...
      .then(data => {
        if (data.success && data.data?.risks) {
          setSection(data.data.risks)
        }
      })
      .catch(() => {})
        ...
        <Card className="border-slate-200 bg-white shadow-lg">
          <CardHeader className="pb-4">
            <div className="flex items-start justify-between">
              <div>
                <Badge className="mb-2" variant="outline">Caution</Badge>
                <CardTitle className="text-2xl">Risks</CardTitle>
              </div>
