...
import type { MVPScope } from '@/types/validation'
...
function MvpContent() {
  ...
  const [section, setSection] = useState<MVPScope | null>(null)
  ...
  useEffect(() => {
    ...
    fetch(`/api/result/${resultId}`)
      ...
      .then(data => {
        if (data.success && data.data?.mvpScope) {
          setSection(data.data.mvpScope)
        }
      })
      .catch(() => {})
        ...
        <Card className="border-slate-200 bg-white shadow-lg">
          <CardHeader className="pb-4">
            <div className="flex items-start justify-between">
              <div>
                <Badge className="mb-2" variant="outline">Build</Badge>
                <CardTitle className="text-2xl">MVP Scope</CardTitle>
              </div>
          ...
          <CardContent className="space-y-6">
            <Subsection title="Core Features">
              <BulletList items={section.coreFeatures} emptyLabel="No core features defined." />
            </Subsection>

            <Field label="Timeline" value={section.timeline} />

            <Subsection title="Success Metrics">
              <BulletList items={section.successMetrics} emptyLabel="No success metrics defined." />
            </Subsection>

            <Subsection title="Resource Needs">
              <BulletList items={section.resourceNeeds} emptyLabel="No resource needs listed." />
            </Subsection>

            <Subsection title="Deferred Capabilities">
              <BulletList items={section.deferredCapabilities} emptyLabel="No deferred capabilities listed." />
            </Subsection>
          </CardContent>
        </Card>
      </div>
