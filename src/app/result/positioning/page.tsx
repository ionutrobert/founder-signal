...
import type { Positioning } from '@/types/validation'
...
function PositioningContent() {
  ...
  const [section, setSection] = useState<Positioning | null>(null)
  ...
  useEffect(() => {
    ...
    fetch(`/api/result/${resultId}`)
      ...
      .then(data => {
        if (data.success && data.data?.positioning) {
          setSection(data.data.positioning)
        }
      })
      .catch(() => {})
        ...
        <Card className="border-slate-200 bg-white shadow-lg">
          <CardHeader className="pb-4">
            <div className="flex items-start justify-between">
              <div>
                <Badge className="mb-2" variant="outline">Brand</Badge>
                <CardTitle className="text-2xl">Positioning</CardTitle>
              </div>
          ...
          <CardContent className="space-y-6">
            ...
            <Subsection title="Differentiators">
              <BulletList items={section.differentiators} emptyLabel="No differentiators listed." />
            </Subsection>

            <Subsection title="Messaging Pillars">
              <BulletList items={section.messagingPillars} emptyLabel="No messaging pillars listed." />
            </Subsection>
