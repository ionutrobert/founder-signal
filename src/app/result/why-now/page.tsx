...
import type { WhyNow } from '@/types/validation'
...
function WhyNowContent() {
  ...
  const [section, setSection] = useState<WhyNow | null>(null)
  ...
  useEffect(() => {
    ...
    fetch(`/api/result/${resultId}`)
      ...
      .then(data => {
        if (data.success && data.data?.whyNow) {
          setSection(data.data.whyNow)
        }
      })
      .catch(() => {})
        ...
        <Card className="border-slate-200 bg-white shadow-lg">
          <CardHeader className="pb-4">
            <div className="flex items-start justify-between">
              <div>
                <Badge className="mb-2" variant="outline">Timing</Badge>
          ...
          <CardContent className="space-y-6">
            <Field label="Timing" value={section.timing} />

            <Subsection title="Market Forces">
              <BulletList items={section.marketForces} emptyLabel="No market forces listed." />
            </Subsection>

            <Subsection title="Enabling Technology">
              <BulletList items={section.enablingTechnology} emptyLabel="No enabling technology listed." />
            </Subsection>

            <Subsection title="Cultural Shift">
              <BulletList items={section.culturalShift} emptyLabel="No cultural shifts listed." />
            </Subsection>
          </CardContent>
        </Card>
      </div>
