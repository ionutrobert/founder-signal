...
import type { ProblemClarity } from '@/types/validation'
...
function ProblemContent() {
  ...
  const [section, setSection] = useState<ProblemClarity | null>(null)
  ...
  useEffect(() => {
    ...
    fetch(`/api/result/${resultId}`)
      ...
      .then(data => {
        if (data.success && data.data?.problemClarity) {
          setSection(data.data.problemClarity)
        }
      })
      .catch(() => {})
        ...
        <Card className="border-slate-200 bg-white shadow-lg">
          <CardHeader className="pb-4">
            <div className="flex items-start justify-between">
              <div>
                <Badge className="mb-2" variant="outline">Problem Space</Badge>
                <CardTitle className="text-2xl">Problem Clarity</CardTitle>
              </div>
          ...
          <CardContent className="space-y-6">
            ...
            <Field label="Severity" value={formatLabel(section.severity)} />
            ...
            <div className="space-y-2">
              ...
              <BulletList items={section.evidence} emptyLabel="No problem evidence provided." />
            </div>

            <Field label="Confidence Level" value={section.confidenceLevel} />
          </CardContent>
        </Card>
      </div>
    </main>
