...
import type { TargetAudience, PersonaProfile } from '@/types/validation'
...
function AudienceContent() {
  ...
  const [section, setSection] = useState<TargetAudience | null>(null)
  ...
  useEffect(() => {
    ...
    fetch(`/api/result/${resultId}`)
      ...
      .then(data => {
        if (data.success && data.data?.targetAudience) {
          setSection(data.data.targetAudience)
        }
      })
      .catch(() => {})
        ...
        <Card className="border-slate-200 bg-white shadow-lg">
          <CardHeader className="pb-4">
            <div className="flex items-start justify-between">
              <div>
                <Badge className="mb-2" variant="outline">Customers</Badge>
          ...
          <CardContent className="space-y-6">
            ...
            <div className="space-y-2">
              ...
              <BulletList items={section.keySegments} emptyLabel="No key segments defined." />
            </div>

            <div className="space-y-2">
              <p className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">Personas</p>
              <PersonaGroup personas={section.personas} />
            </div>
          </CardContent>
        </Card>
      </div>
