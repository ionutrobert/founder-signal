...
async function testModel(modelId) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return { modelId, status: 'ERROR', latency, error: `${response.status} ${error.substring(0, 100)}` };
    }
    ...
    if (content && content.length > 0) {
      return { modelId, status: 'OK', latency, response: content.substring(0, 50) };
    } else {
...
async function main() {
  ...
  if (working.length > 0) {
    console.log('Working models (fastest first):');
